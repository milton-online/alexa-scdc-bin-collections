...
describe ('BinCollection', function() {
    ...
    describe('getDateSpeech()', function(){
        it('slipped collection', function() {
            slippedExample.getDateSpeech().should.equal(
                `on Sun Jan 19 .${messages.SLIPPED_COLLECTION}`
            )
        })
        it('today', function() {
            slippedExample.date.setTime(Date.now())
            slippedExample.slippedCollection = false
            slippedExample.getDateSpeech().should.equal(`today.${messages.DID_YOU_MISS_IT}`)
        })
        it('tomorrow', function() {
            slippedExample.date.setTime(Date.now() + 86400000)
            slippedExample.slippedCollection = false
            slippedExample.getDateSpeech().should.equal(
                "tomorrow.  Better get that bin out!"
            )
        })
    ...
    describe('getColoursSpeech()', function(){
        ...
        it('two bins', function () {
            multipleExample.getColoursSpeech().should.equal(
                "green and blue bins"
            )
        })
        ...
        it('three bins', function () {
            tripleExample.getColoursSpeech().should.equal(
                "green, blue and black bins"
            )
        })
    })
})
